package com.mmall.alipay.trade;
...
public class PayAction {
    private static Log log = LogFactory.getLog(PayAction.class);
...
//        PayAction payAction = new PayAction();
        ...
        // 系统商商测试交易保障接口api
        //        payAction.monitor_sys();

        // POS厂商测试交易保障接口api
        //        payAction.monitor_pos();

        // 测试交易保障接口调度
        //        payAction.monitor_schedule_logic();

        // 测试当面付2.0支付（使用未集成交易保障接口的当面付2.0服务）
//                payAction.trade_pay(tradeService);

        // 测试查询当面付2.0交易
//                payAction.trade_query();

        // 测试当面付2.0退货
        //        payAction.trade_refund();

        // 测试当面付2.0生成支付二维码
//        payAction.trade_precreate();
    }

    // 测试系统商交易保障调度
    public static void monitor_schedule_logic() {
        // 启动交易保障线程
    ...
    public static void monitor_sys() {
    ...
    public static void monitor_pos() {
    ...
    // 测试当面付2.0支付
    public static void trade_pay(AlipayTradeService service) {
    ...
    // 测试当面付2.0查询订单
    public static AlipayF2FQueryResult trade_query(String orderNo) {
    ...
    // 测试当面付2.0退款
    public static void trade_refund() {
    ...
    // 测试当面付2.0生成支付二维码
    public static AlipayF2FPrecreateResult trade_precreate(Order order,List<OrderItem> orderItemList) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
...
//                            + (long) (Math.random() * 10000000L);
        ...
        String subject = new StringBuffer("cls当面付扫码消费, ").append("订单编号：").append(order.getOrderNo()).toString();
        ...
        // 卖家支付宝账号ID，用于支持一个签约账号下支持打款到不同的收款账号，(打款到sellerId对应的支付宝账号)
        // 如果该字段为空，则默认为与支付宝签约的商户的PID，也就是appid对应的PID  （它好像写错了，应该是 pid）
        String sellerId = "";

        // 订单描述，可以对交易或商品进行一个详细地描述，比如填写"购买商品2件共15.00元"
        String body = new StringBuffer("购买商品共：").append(orderItemList.size()+"件 ")
                .append("总计: ").append(totalAmount).append(" 元。").toString();
...
//        goodsDetailList.add(goods2);
        for(OrderItem orderItem:orderItemList){
            GoodsDetail goodsDetail=GoodsDetail.newInstance(String.valueOf(orderItem.getProductId()),orderItem.getProductName(),
                    orderItem.getTotalPrice().longValue()*100,orderItem.getQuantity());
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
            ...
            .setTimeoutExpress(timeoutExpress)
                            .setNotifyUrl("http://47.107.38.7:8080/mmall/payInfo/alipayCallback")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
//                String filePath = String.format("C:/Users/16963/Desktop/qrImg/qr-%s.png",
//                    response.getOutTradeNo());
//                log.info("filePath:" + filePath);
//                File imageFile=ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);  // 生成二維碼,并保存到filePath（必須的存在）
